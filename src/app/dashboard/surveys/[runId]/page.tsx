...
// ---------------------------------------------------------------------------
// /dashboard/surveys/[runId] — survey management (authenticated)
// ---------------------------------------------------------------------------
...
function SurveyManageContent() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Try with segmentation columns, fallback to basic
    ...
    try {
      ...
      if (invitesWithSegErr) {
        ...
        if (invitesBasicErr || !invitesBasic) {
          throw new Error(
            invitesBasicErr?.message || "Could not load invites."
          );
        }
      ...
      const sorted = [...responses].sort((a, b) =>
        (a.invite_token + a.question_id).localeCompare(
          b.invite_token + b.question_id
        )
      );
  ...
  const downloadSummaryCsv = async () => {
    ...
    try {
      ...
      if (trustErr || !trust) {
        throw new Error(
          trustErr?.message || "Could not load trust scores."
        );
      }
      ...
      if (countsErr || !counts) {
        throw new Error(
          countsErr?.message || "Could not load response counts."
        );
      }
      ...
      if (dimsErr || !dims) {
        throw new Error(
          dimsErr?.message || "Could not load dimension scores."
        );
      }
  ...
  const resultsHref = `/dashboard/surveys/${runId}/results`;
  const surveyLinks = invites
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <a
                ...
                href={
                  ...
                  "subject=" +
                  encodeURIComponent(
                    `Reminder: ${run?.title || "TrustOrg survey"}`
                  ) +
                  "&body=" +
                  encodeURIComponent(
