...
export default function AccountSettingsPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Account info card */}
      <div className="border border-verisum-grey rounded-lg p-6 space-y-5">
        <div className="flex items-center justify-between">
          ...
          {!editing && (
            <button
              ...
              className="text-sm text-verisum-blue hover:text-verisum-black transition-colors"
            >
              Edit
            </button>
          )}
        ...
        {editing ? (
          <div className="space-y-4">
            <div className="space-y-1">
              <label className="text-sm font-medium text-verisum-grey">Full name</label>
              <input
            ...
            <div className="space-y-1">
              <label className="text-sm font-medium text-verisum-grey">Organisation name</label>
              <input
                className="w-full border border-verisum-grey rounded px-3 py-2 text-sm"
                value={companyName}
            ...
            <div className="space-y-1">
              <label className="text-sm font-medium text-verisum-grey">Organisation size</label>
              <input
                className="w-full border border-verisum-grey rounded px-3 py-2 text-sm"
                value={companySize}
            ...
            <div className="space-y-1">
              <label className="text-sm font-medium text-verisum-grey">Role</label>
              <input
                className="w-full border border-verisum-grey rounded px-3 py-2 text-sm"
                value={role}
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={handleSave}
                disabled={saving}
                className="px-4 py-2 rounded bg-verisum-blue text-verisum-white text-sm font-semibold hover:bg-[#2a7bb8] disabled:opacity-50"
              >
            ...
            {saveMsg && (
              <div
                className={`text-sm ${
                  saveMsg === "Profile updated" ? "text-verisum-green" : "text-verisum-red"
                }`}
              >
                {saveMsg}
              </div>
          ...
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4 text-sm">
            <div>
              <dt className="text-verisum-grey">Full name</dt>
              <dd className="font-medium">{profile?.full_name || "—"}</dd>
            </div>
            <div>
              <dt className="text-verisum-grey">Organisation</dt>
              <dd className="font-medium">{profile?.company_name || "—"}</dd>
            </div>
            <div>
              <dt className="text-verisum-grey">Organisation size</dt>
              <dd className="font-medium">{profile?.company_size || "—"}</dd>
            </div>
            <div>
              <dt className="text-verisum-grey">Role</dt>
              <dd className="font-medium">{profile?.role || "—"}</dd>
            </div>
            <div>
              <dt className="text-verisum-grey">Email</dt>
              <dd className="font-medium">{user?.email || "—"}</dd>
            </div>
            <div>
              <dt className="text-verisum-grey">Account created</dt>
      ...
      <div className="border border-verisum-grey rounded-lg p-6 space-y-4">
        <h2 className="text-lg font-semibold text-verisum-black">Plan & usage</h2>

        <dl className="grid grid-cols-1 sm:grid-cols-3 gap-x-8 gap-y-4 text-sm">
          <div>
            <dt className="text-verisum-grey">Current plan</dt>
            <dd className="font-medium capitalize flex items-center gap-2">
              {planLabel}
              <span className="text-xs px-2 py-0.5 rounded-full bg-verisum-blue/10 text-verisum-blue font-medium capitalize">
                {planLabel}
              </span>
            </dd>
          </div>
          <div>
            <dt className="text-verisum-grey">Surveys used</dt>
            <dd className="font-medium">
              {surveyCount !== null
          ...
          <div>
            <dt className="text-verisum-grey">Systems assessed</dt>
            <dd className="font-medium">
              {systemCount !== null
        ...
        <div className="flex flex-wrap items-center gap-3 pt-2">
          {planLabel !== "enterprise" && (
            <a
              href="/upgrade"
              className="px-4 py-2 rounded bg-verisum-blue text-verisum-white text-sm font-semibold hover:bg-[#2a7bb8] inline-block"
            >
              Upgrade plan
            </a>
          )}
